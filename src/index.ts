...
export class Web3MarketplacePlugin extends Web3PluginBase {
  ...
  private getSender = async () => {
    ...
    if (!sender) {
      throw new Error("No account found, please connect to a wallet provider or set a default account - 'web3.config.defaultAccount = <your address>'");
    }
    return sender;
  }
 ...
 */
  public async listItem(contract: Contract<typeof marketplaceABI>, nftContract: string, tokenId: string, price: string, deadline: string): Promise<Receipt> {
    try {
   ...
   * @returns {Promise<Receipt>} - Returns the transaction receipt object.
   */
  public async listItems(contract: Contract<typeof marketplaceABI>, nftContracts: string[], tokenIds: string[], amounts: string[], prices: string[], deadlines: string[]): Promise<Receipt> {
    try {
      const sender = await this.getSender();

      const receipt = await contract.methods.listItems(nftContracts, tokenIds, amounts, prices, deadlines).send({
          from: sender
      });
 ...
 * @returns {Promise<Receipt>} - Returns the transaction receipt object.
 */
public async cancelExpiredListings(contract: Contract<typeof marketplaceABI>, itemIds: string[]): Promise<Receipt> {
  try {
   ...
   * @returns {Promise<Receipt>} - Returns the transaction receipt object.
   */
  public async purchaseItem(contract: Contract<typeof marketplaceABI>, itemId: string, price: string) {
    try {
      const sender = await this.getSender()
